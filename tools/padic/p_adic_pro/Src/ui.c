...
static bool parse_prime_base(const char* text, int* p_out) {
    char* end = NULL;
    long parsed = strtol(text, &end, 10);
    while (end != NULL && isspace((unsigned char)*end)) end++;
    if (end == NULL || *end != '\0' || parsed < 2 || parsed > 2147483647L) return false;
    if (!is_prime((int)parsed)) return false;
    *p_out = (int)parsed;
    return true;
}
...
void interactive_padic() {
    ...
    while (1) {
        ...
        if (!parse_prime_base(last_space + 1, &p)) {
            printf("%s\n", lang_get(STR_ERR_P_VALUE));
            continue;
        }

        RationalNumber* rat = parse_rational_strict(num_str);
        if (rat == NULL) {
...
void expression_to_padic() {
    ...
    while (1) {
        ...
        RationalNumber rat_result;
        if (!evaluate_ast_safe(ast, &rat_result)) {
            printf("%s\n", lang_get(STR_ERR_EXPR_PARSE));
            free_ast(ast);
            continue;
        }
