...
PAdicNumber* padic_sqrt(PAdicNumber* n, int p, int digits) {
    ...
    for (int k = 1; k < digits; k++) {
        ...
        for (int j = 0; j <= k; j++) {
            long long sum = carry;
            for (int i = 0; i <= j; i++) {
                int left = (i < k) ? result->digits[i] : 0;
                int right_index = j - i;
                int right = (right_index < k) ? result->digits[right_index] : 0;
                sum += (long long)left * right;
            }
            square_digit = (int)(sum % p);
            carry = sum / p;
        }

        int unit_digit = 0;
        int idx = v + k - n->min_power;
        if (idx >= 0 && idx < n->length) unit_digit = n->digits[idx];

        int fprime = (2 * result->digits[0]) % p;
        int fprime_inv = modular_inverse(fprime, p);
        int error_digit = (unit_digit - square_digit) % p;
        if (error_digit < 0) error_digit += p;
        result->digits[k] = (int)(((long long)error_digit * fprime_inv) % p);
    }
