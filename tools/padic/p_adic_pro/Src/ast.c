...
ASTNode* parse(const char* input) {
    ...
    if (state.current_token.type != TOKEN_END) {
        fprintf(stderr, "Error: Unexpected trailing input\n");
        free_ast(node);
        return NULL;
    }
...
Token get_next_token(ParserState* state) {
    ...
    switch (current) {
        ...
        case '^': token.type = TOKEN_POWER;  state->position++; break;
...
ASTNode* parse_power(ParserState* state) {
    ...
    if (state->current_token.type == TOKEN_POWER) {
        ...
        node = create_binary_node(NODE_POW, node, right);
    }
...
Token read_number(ParserState* state) {
    ...
    if (is_variable_char(state->input[state->position], true)) {
        while (isdigit(state->input[state->position])) {
            state->position++;
        }
        token.type = TOKEN_ERROR;
